...
import { DrawerEmit, DrawerProps, UseDrawerFn } from './types'
...
export function useDrawer(props: DrawerProps, emit: DrawerEmit): UseDrawerFn {
  ...
  const handleClickOutside = (e: PointerEvent) => {
    ...
    const isClickModalOverlay = Array.from(modalOverlay).filter(item =>
      composedPath.includes(item)
    )
    const isClickModal = Array.from(modal).filter(item =>
      composedPath.includes(item)
    )
  ...
  setTimeout(() => {
    onClickOutside(drawerRef, handleClickOutside, {
      capture: false,
      ignore: [overlayRef]
    })
  })
