  ...
  setup(props: AvatarProps) {
    ...
    const setDisplayName = (nameValue: string, widthValue: number): void => {
      ...
      if (nameValue.length < 2) {
        ...
        if (/^[\u4e00-\u9fa5]/.test(nameValue)) {
          ...
          if (/[_ -]/.test(nameValue)) {
            const strbefore = nameValue.split(/_|-|\s+/)[0]
            const strafter = nameValue.split(/_|-|\s+/)[1]
            nameDisplay.value =
              strbefore.substr(0, 1).toUpperCase() +
              strafter.substr(0, 1).toUpperCase()
          } else {
            // 一个英文名的情况显示前两个字母
    ...
    return () => {
      ...
      const hasNameDisplay =
        !imgSrc.value && !isNobody.value && nameDisplay.value?.length !== 0
          ? nameElement
          : null
      ...
      const noBody =
        (!imgSrc.value && isNobody.value) || isErrorImg.value
          ? noBodyElement
          : null
      return (
        <span class={ns.b()}>
